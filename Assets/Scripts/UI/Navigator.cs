...
public class Navigator : MonoBehaviour
{
    ...
    private void FixedUpdate()
    {
        ...
        {
            bigArrow.transform.position = transform.position + (_gate.transform.position - transform.position).normalized * 6.0f;
        ...
        foreach (var arrow in _minArrows)
        {
            if (Targets[i].active)
            {
                arrow.transform.position = transform.position + (Targets[i].transform.position - transform.position).normalized * 5.0f;
